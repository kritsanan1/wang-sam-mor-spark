...
const WelcomeImage: React.FC<WelcomeImageProps> = ({ 
  ...
  if (error) {
    return (
      <div className="w-full min-h-[300px] bg-wang-lightGray flex items-center justify-center rounded-lg">
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-lg">
      ...
      <div className={`relative aspect-[16/9] overflow-hidden ${loading ? 'opacity-0' : 'animate-scale-in opacity-100'}`}>
        {imageUrl && (
          <img
            src={imageUrl}
            alt={description || "Welcome image"}
            className="w-full h-full object-cover"
            onLoad={() => setLoading(false)}
          />
        )}
        
        {/* Image overlay with gradient and description */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex flex-col justify-end p-4 md:p-6">
          {description && (
            <p className="text-white text-base md:text-lg font-medium max-w-3xl animate-fade-in">
              {description}
            </p>
          )}
        </div>
